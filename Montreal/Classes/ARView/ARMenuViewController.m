...
- (id)initWithTitle:(NSString *)title message:(NSString *)msg
{
    if (self = [super init])
    {
        if ( IDIOM == IPAD )
            ...
            self.view = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, 80)] ;
...
- (void)onListView:(id)sender
{
    ...
    MapPlaceListViewController * viewController = [sb instantiateViewControllerWithIdentifier:@"mapPlaceListViewController"];
    viewController.delegate = _onGotoDelegate;
    viewController.baseimageurl = self.baseimageurl;
...
- (void)onMapView:(id)sender
{
    ...
    UIStoryboard *sb = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    LocationMapViewController * viewController = [sb instantiateViewControllerWithIdentifier:@"locationMapViewController"];
    [superViewController.navigationController pushViewController:viewController animated:true];
